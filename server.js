...
function emailActivity(email, name, event) {
    ...
    const msg = {
        ...
        //text: `${name}, This week's Findr activity is ${event}!!`,
        html: `${name}, This week's Findr activity is ${event}!! <br /> Thanks for using Findr! We'll see you next week!`,
    };
...
// Find Event with the most likes?
...
function updateInterests(interests) {
    ...
    interests.array.forEach(item => {
        events[item].badgeCount += 1;
        console.log("Interest value incremented for " + events[item].eventName + ". There are " + event[item].badgeCount + " total people interested");
    });
}
